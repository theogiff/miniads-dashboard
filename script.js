...
function configureExtensionCta() {
  if (extensionInstallBtn) {
    extensionInstallBtn.href = MINIADS_EXTENSION_INSTALL_URL;
  }
...
// ===== PREVIEW TOOL (intégré dans le dashboard) =====
(function previewToolInit() {
  const PV_API_KEY = 'AIzaSyDekxhLAwMVLC56TRRIWPZ-gHdfprd4Ftk';
  const PV_DEFAULT_TITLE = 'Votre titre de video';
  const PV_DEFAULT_CHANNEL = 'Votre chaine YouTube';

  let pvThumbnail = null;
  let pvTitle = PV_DEFAULT_TITLE;
  let pvChannel = PV_DEFAULT_CHANNEL;
  let pvPosition = 0;
  let pvAllVideos = [];
  let pvChannelData = null;
  let pvVideoSources = [];

  function pvSave() {
    try {
      localStorage.setItem('miniads_preview', JSON.stringify({
        thumbnail: pvThumbnail, title: pvTitle, channel: pvChannel,
        channelData: pvChannelData, videoSources: pvVideoSources
      }));
    } catch(e) {}
  }
  ...
  function pvLoad() {
    try {
      const d = JSON.parse(localStorage.getItem('miniads_preview') || '{}');
      if (d.thumbnail) { pvThumbnail = d.thumbnail; pvShowThumb(pvThumbnail); }
      if (d.title) { pvTitle = d.title; const ti = document.getElementById('previewTitleInput'); if (ti) ti.value = pvTitle; }
      if (d.channel) pvChannel = d.channel;
      if (d.channelData) { pvChannelData = d.channelData; pvShowChannelChip(d.channelData, 'previewSelectedChannelPreview'); }
      if (Array.isArray(d.videoSources)) { pvVideoSources = d.videoSources; pvRenderSourcePreviews(); pvUpdateSourcePlaceholder(); }
      const ph = document.getElementById('previewChannelSearchPlaceholder');
      if (ph) ph.textContent = pvChannel === PV_DEFAULT_CHANNEL ? 'Rechercher une chaine...' : pvChannel;
    } catch(e) {}
  }

  function pvShowThumb(src) {
    const img = document.getElementById('previewUserThumbnail');
    const prompt = document.getElementById('previewUploadPrompt');
    const container = document.getElementById('previewThumbnailContainer');
  ...
  // Thumbnail upload
  const pvThumbContainer = document.getElementById('previewThumbnailContainer');
  const pvThumbInput = document.getElementById('previewThumbnailInput');
  if (pvThumbContainer && pvThumbInput) {
    pvThumbContainer.addEventListener('click', () => pvThumbInput.click());
    pvThumbInput.addEventListener('change', (e) => {
      const f = e.target.files && e.target.files[0];
      if (!f) return;
      const r = new FileReader();
      r.onload = (ev) => { pvThumbnail = ev.target.result; pvShowThumb(pvThumbnail); pvSave(); pvRenderGrid(); };
      r.readAsDataURL(f);
    });
  }

  // Title input
  const pvTitleInput = document.getElementById('previewTitleInput');
  if (pvTitleInput) {
    pvTitleInput.addEventListener('input', (e) => {
      pvTitle = e.target.value || PV_DEFAULT_TITLE;
      pvSave(); pvRenderGrid();
    });
  }

  // Modal helpers
  function pvOpenModal(modalId, inputId) {
    const m = document.getElementById(modalId);
    if (!m) return;
    m.style.display = 'flex';
    requestAnimationFrame(() => m.classList.add('show'));
    if (inputId) { const inp = document.getElementById(inputId); if (inp) setTimeout(() => inp.focus(), 120); }
  }

  function pvCloseModal(modalId) {
    const m = document.getElementById(modalId);
    if (!m) return;
  ...
  // Modal triggers
  const pvChannelBtn = document.getElementById('previewChannelSearchBtn');
  if (pvChannelBtn) pvChannelBtn.addEventListener('click', () => pvOpenModal('previewChannelSearchModal', 'previewChannelSearchInput'));

  const pvSourceBtn = document.getElementById('previewVideoSourceSearchBtn');
  if (pvSourceBtn) pvSourceBtn.addEventListener('click', () => pvOpenModal('previewVideoSourceSearchModal', 'previewVideoSourceSearchInput'));

  document.querySelectorAll('.preview-label-action').forEach(l => {
    l.addEventListener('click', () => pvOpenModal(l.dataset.modalId, l.dataset.inputId));
  });

  document.querySelectorAll('.preview-modal-overlay').forEach(m => {
    m.addEventListener('click', (e) => { if (e.target === m) pvCloseModal(m.id); });
  });

  document.querySelectorAll('[data-close-preview-modal]').forEach(b => {
    b.addEventListener('click', () => pvCloseModal(b.dataset.closePreviewModal));
  });

  // Channel search
  function pvFormatSubs(c) {
    const n = parseInt(c);
    if (n >= 1000000) return (n/1000000).toFixed(1) + 'M';
  ...
  async function pvSearchChannels(query, container, onSelect) {
    try {
      const r = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&type=channel&q=${encodeURIComponent(query)}&maxResults=10&key=${PV_API_KEY}`);
      if (!r.ok) { container.innerHTML = '<div class="preview-results-empty"><div class="preview-empty-text">Erreur lors de la recherche</div></div>'; return; }
      const data = await r.json();
      if (!data.items || !data.items.length) { container.innerHTML = '<div class="preview-results-empty"><div class="preview-empty-text">Aucune chaine trouvee</div></div>'; return; }

      const ids = data.items.map(i => i.snippet.channelId).join(',');
      const sr = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=statistics&id=${ids}&key=${PV_API_KEY}`);
      ...
      data.items.forEach(item => {
        const ch = { id: item.snippet.channelId, title: item.snippet.title, thumbnail: item.snippet.thumbnails.default.url, subscribers: sm[item.snippet.channelId]?.subscriberCount || '0' };
        const div = document.createElement('div');
        div.className = 'preview-result-item';
        div.innerHTML = `<img src="${ch.thumbnail}" alt="${ch.title}" class="preview-result-avatar"><div><div class="preview-result-name">${ch.title}</div><div class="preview-result-subs">${pvFormatSubs(ch.subscribers)} abonnes</div></div>`;
        div.addEventListener('click', () => onSelect(ch));
        container.appendChild(div);
      });
    } catch(e) {
      container.innerHTML = '<div class="preview-results-empty"><div class="preview-empty-text">Erreur lors de la recherche</div></div>';
    }
  }

  // Channel search input
  const pvChSearchInput = document.getElementById('previewChannelSearchInput');
  if (pvChSearchInput) {
    let t;
    pvChSearchInput.addEventListener('input', (e) => {
      const q = e.target.value.trim();
      const rc = document.getElementById('previewChannelSearchResults');
      if (q.length < 2) { rc.innerHTML = '<div class="preview-results-empty"><div class="preview-empty-text">Recherchez une chaine YouTube</div></div>'; return; }
      rc.innerHTML = '<div class="preview-results-empty"><div class="preview-empty-text">Recherche en cours...</div></div>';
      clearTimeout(t);
      t = setTimeout(() => {
        pvSearchChannels(q, rc, (ch) => {
          pvChannelData = ch; pvChannel = ch.title;
          const ph = document.getElementById('previewChannelSearchPlaceholder');
          if (ph) ph.textContent = ch.title;
          pvShowChannelChip(ch, 'previewSelectedChannelPreview');
          pvCloseModal('previewChannelSearchModal');
          pvChSearchInput.value = '';
          pvSave(); pvRenderGrid();
        });
      }, 300);
    });
  ...
  // Video source search input
  const pvSrcSearchInput = document.getElementById('previewVideoSourceSearchInput');
  if (pvSrcSearchInput) {
    let t;
    pvSrcSearchInput.addEventListener('input', (e) => {
      const q = e.target.value.trim();
      const rc = document.getElementById('previewVideoSourceSearchResults');
      if (q.length < 2) { rc.innerHTML = '<div class="preview-results-empty"><div class="preview-empty-text">Recherchez une chaine YouTube</div></div>'; return; }
      rc.innerHTML = '<div class="preview-results-empty"><div class="preview-empty-text">Recherche en cours...</div></div>';
      clearTimeout(t);
      t = setTimeout(() => {
        pvSearchChannels(q, rc, async (ch) => {
          if (!pvVideoSources.some(c => c.id === ch.id)) {
            pvVideoSources.push(ch);
            pvUpdateSourcePlaceholder(); pvRenderSourcePreviews(); pvSave();
            await pvCreateFeed(true);
          }
          pvCloseModal('previewVideoSourceSearchModal');
          pvSrcSearchInput.value = '';
        });
      }, 300);
    });
  }

  function pvShowChannelChip(ch, containerId) {
    const c = document.getElementById(containerId);
    if (!c) return;
    c.innerHTML = `<div class="preview-channel-preview"><img src="${ch.thumbnail}" alt="${ch.title}" class="preview-channel-avatar"><div class="preview-channel-info"><div class="preview-channel-name">${ch.title}</div><div class="preview-channel-subs">${pvFormatSubs(ch.subscribers)} abonnes</div></div></div>`;
  }

  function pvUpdateSourcePlaceholder() {
    const ph = document.getElementById('previewVideoSourceSearchPlaceholder');
    if (!ph) return;
    if (pvVideoSources.length === 0) ph.textContent = 'Rechercher une chaine...';
    else if (pvVideoSources.length === 1) ph.textContent = pvVideoSources[0].title;
    else ph.textContent = pvVideoSources.length + ' chaines selectionnees';
  }

  function pvRenderSourcePreviews() {
    const c = document.getElementById('previewSelectedVideoSourcePreview');
    if (!c) return;
    if (!pvVideoSources.length) { c.innerHTML = ''; return; }
    const list = document.createElement('div');
    list.className = 'preview-channel-preview-list';
    pvVideoSources.forEach(ch => {
      const item = document.createElement('div');
      item.className = 'preview-channel-preview';
      item.innerHTML = `<img src="${ch.thumbnail}" alt="${ch.title}" class="preview-channel-avatar"><div class="preview-channel-info"><div class="preview-channel-name">${ch.title}</div><div class="preview-channel-subs">${pvFormatSubs(ch.subscribers)} abonnes</div></div><button class="preview-channel-remove" data-remove-src="${ch.id}">&times;</button>`;
      list.appendChild(item);
    });
    c.innerHTML = '';
    c.appendChild(list);
    c.querySelectorAll('[data-remove-src]').forEach(btn => {
      btn.addEventListener('click', async () => {
        pvVideoSources = pvVideoSources.filter(s => s.id !== btn.dataset.removeSrc);
        pvUpdateSourcePlaceholder(); pvRenderSourcePreviews(); pvSave();
        await pvCreateFeed(true);
      });
    });
  ...
  // Position buttons
  const pvRandomBtn = document.getElementById('previewRandomPosBtn');
  if (pvRandomBtn) pvRandomBtn.addEventListener('click', () => { if (pvAllVideos.length) { pvPosition = Math.floor(Math.random() * (pvAllVideos.length + 1)); pvRenderGrid(); } });

  const pvTopBtn = document.getElementById('previewTopPosBtn');
  if (pvTopBtn) pvTopBtn.addEventListener('click', () => { pvPosition = 0; pvRenderGrid(); });
  ...
  document.querySelectorAll('.preview-view-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      btn.classList.add('active');
      const tool = document.querySelector('.preview-tool');
      if (!tool) return;
      tool.classList.remove('mobile-view', 'tab-view');
      if (btn.dataset.previewView === 'mobile') tool.classList.add('mobile-view');
      else if (btn.dataset.previewView === 'tab') tool.classList.add('tab-view');
    });
  });

  // Refresh & Clear
  const pvRefreshBtn = document.getElementById('previewRefreshBtn');
  if (pvRefreshBtn) pvRefreshBtn.addEventListener('click', async () => { pvPosition = 0; await pvCreateFeed(true); });

  const pvClearBtn = document.getElementById('previewClearBtn');
  if (pvClearBtn) pvClearBtn.addEventListener('click', async () => {
    pvThumbnail = null; pvTitle = PV_DEFAULT_TITLE; pvChannel = PV_DEFAULT_CHANNEL;
    pvPosition = 0; pvChannelData = null; pvVideoSources = []; pvAllVideos = [];
    const img = document.getElementById('previewUserThumbnail');
    const prompt = document.getElementById('previewUploadPrompt');
    const container = document.getElementById('previewThumbnailContainer');
    ...
    const ti = document.getElementById('previewTitleInput'); if (ti) ti.value = '';
    const cp = document.getElementById('previewChannelSearchPlaceholder'); if (cp) cp.textContent = 'Rechercher une chaine...';
    const sp = document.getElementById('previewVideoSourceSearchPlaceholder'); if (sp) sp.textContent = 'Rechercher une chaine...';
    const scp = document.getElementById('previewSelectedChannelPreview'); if (scp) scp.innerHTML = '';
    const svp = document.getElementById('previewSelectedVideoSourcePreview'); if (svp) svp.innerHTML = '';
    localStorage.removeItem('miniads_preview');
    await pvCreateFeed(true);
  });

  // YouTube API helpers
  function pvParseDuration(d) {
    const m = (d || 'PT0M0S').match(/PT(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?/);
    return parseInt(m[1]||0)*3600 + parseInt(m[2]||0)*60 + parseInt(m[3]||0);
  }

  function pvFormatDuration(d) {
    const m = (d || 'PT0M0S').match(/PT(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?/);
  ...
  function pvTimeAgo(pub) {
    const diff = Math.floor((new Date() - new Date(pub)) / (1000*60*60*24));
    if (diff === 0) return "aujourd'hui";
    if (diff === 1) return '1 jour';
    if (diff < 7) return diff + ' jours';
    if (diff < 30) return Math.floor(diff/7) + ' semaine' + (Math.floor(diff/7) > 1 ? 's' : '');
  ...
  const pvColors = ['#ff6b6b','#4ecdc4','#45b7d1','#f9ca24','#6c5ce7','#fd79a8','#fdcb6e','#00b894'];
  function pvGetColor(name) {
    let h = 0;
    for (let i = 0; i < name.length; i++) h = name.charCodeAt(i) + ((h << 5) - h);
    return pvColors[Math.abs(h) % pvColors.length];
  }

  async function pvFetchForChannel(channelId) {
    const sr = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${channelId}&order=date&type=video&maxResults=50&key=${PV_API_KEY}&rnd=${Math.random()}`);
    if (!sr.ok) return null;
    const sd = await sr.json();
    const ids = sd.items.map(i => i.id.videoId).filter(Boolean).join(',');
    if (!ids) return [];
    const dr = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&id=${ids}&key=${PV_API_KEY}&rnd=${Math.random()}`);
    if (!dr.ok) return null;
    const dd = await dr.json();
    return dd.items || [];
  }

  async function pvFetchVideos() {
    try {
      if (pvVideoSources.length > 0) {
        const all = [];
        for (const src of pvVideoSources) {
          const vids = await pvFetchForChannel(src.id);
          if (vids && vids.length) all.push(...vids);
        }
        return all;
      }
      const r = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&chart=mostPopular&regionCode=FR&maxResults=50&key=${PV_API_KEY}&rnd=${Math.random()}`);
      if (!r.ok) return null;
  ...
  function pvCreateCard(v) {
    const card = document.createElement('div');
    card.className = 'preview-video-card';
    if (v.isCustom) card.classList.add('preview-custom-video');
    const initial = v.channel.charAt(0).toUpperCase();
    let avatarHTML = v.channelAvatar
      ? `<img src="${v.channelAvatar}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`
      : initial;
    card.innerHTML = `
      <div class="preview-video-thumb">
        ${v.thumbnail ? `<img src="${v.thumbnail}" alt="${v.title}">` : `<div style="width:100%;height:100%;background:${v.bgColor||'#272727'}"></div>`}
        <span class="preview-video-duration">${v.duration}</span>
      </div>
      <div class="preview-video-info">
        <div class="preview-video-avatar" style="background:${v.channelAvatar ? 'transparent' : pvGetColor(v.channel)};overflow:hidden;">${avatarHTML}</div>
        <div class="preview-video-details">
          <div class="preview-video-title">${v.title}${v.isCustom ? '<span class="preview-badge">VOTRE VIDEO</span>' : ''}</div>
          <div class="preview-video-meta">${v.channel}<br>${v.views} &bull; il y a ${v.time}</div>
        </div>
      </div>`;
    return card;
  }
  ...
  function pvRenderGrid() {
    const grid = document.getElementById('previewVideoGrid');
    if (!grid) return;
    grid.innerHTML = '';
    const custom = pvCreateCard({
      thumbnail: pvThumbnail, title: pvTitle, channel: pvChannel,
      channelAvatar: pvChannelData?.thumbnail, views: '0 vues', time: 'maintenant', duration: '0:00', isCustom: true
    });
    pvAllVideos.forEach((v, i) => {
      if (i === pvPosition) grid.appendChild(custom);
      grid.appendChild(v);
    });
    if (pvPosition >= pvAllVideos.length) grid.appendChild(custom);
  }

  async function pvCreateFeed(forceShuffle) {
    pvAllVideos = [];
    const vids = await pvFetchVideos();
    if (vids && vids.length) {
      const longOnly = vids.filter(v => pvParseDuration(v.contentDetails?.duration) >= 600);
      const shuffled = pvShuffle(longOnly);
      const toShow = forceShuffle ? pvShuffle(shuffled) : shuffled;
      toShow.forEach(v => {
        pvAllVideos.push(pvCreateCard({
          thumbnail: v.snippet.thumbnails.medium.url,
          title: v.snippet.title,
          channel: v.snippet.channelTitle,
          views: pvFormatViews(v.statistics?.viewCount || 0) + ' vues',
          time: pvTimeAgo(v.snippet.publishedAt),
          duration: pvFormatDuration(v.contentDetails.duration),
          isCustom: false
        }));
      });
    }
